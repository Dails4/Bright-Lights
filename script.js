...
function updateProgress() {
  ...
  const durationIsValid = !isNaN(duration) && isFinite(duration);
  const curMinutes = Math.floor(currentTime / 60),
        curSeconds = Math.floor(currentTime % 60),
        durMinutes = durationIsValid ? Math.floor(duration / 60) : 0,
        durSeconds = durationIsValid ? Math.floor(duration % 60) : 0;
